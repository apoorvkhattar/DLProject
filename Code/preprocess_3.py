...
def _create_tsv(data_dir, quesfile, ansfile, outfile, istrain, ansid=None):
    ...
    with open(outfile, 'w') as out:
            for q, qid, i, a in zip(ques, quesid, imgid, ans):
                    if (a == 0 and idx_0 < img_num):
                        out.write('\t'.join([str(qid), q, str(i), str(a)]) + '\n')
                        idx_0 += 1
                    if (a == 1 and idx_1 < img_num):
                        idx_1 += 1
                        out.write('\t'.join([str(qid), q, str(i), str(a)]) + '\n')
    return ansid
...
def preprocess(data_dir, train_ques_file, train_ans_file, val_ques_file, val_ans_file):
    ...
    print("TRAIN TSV")
    ansid = _create_tsv(data_dir=data_dir, quesfile=train_ques_file, ansfile=train_ans_file, istrain = True, outfile=train_tsv_file)
    # create val tsv using ans-to-idx mapping made on train data
    print("VAL TSV")
    _create_tsv(data_dir=data_dir, quesfile=val_ques_file, ansfile=val_ans_file, outfile=val_tsv_file, istrain = False, ansid=ansid)
